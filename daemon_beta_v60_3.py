...
I18N_TC = {
    ...
    "Napoli": "拿坡里", "Roma": "羅馬", "Lazio": "拉齊奧", "Atalanta": "亞特蘭大",
...
def run():
    ...
    for m in matches:
        ...
        # STRICT FILTER: Only allow Top 5 + CL
        if code not in ['PL', 'PD', 'SA', 'BL1', 'FL1', 'CL']: 
            continue
...
import random # For simulation
        ...
        # Events Simulation (Test Mode)
        if status == 'IN_PLAY':
            # Force inject dummy events if empty, just to show UI capabilities as requested
            if not events:
                events = [
                    {"minute": 12, "type": "goal", "text": {"en": "Goal: Haaland", "tc": "進球: 哈蘭德", "sc": "进球: 哈兰德"}},
                    {"minute": 34, "type": "card", "card": "yellow", "text": {"en": "Yellow: Rodri", "tc": "黃牌: 羅德里", "sc": "黄牌: 罗德里"}},
                    {"minute": 56, "type": "card", "card": "red", "text": {"en": "Red: Casemiro", "tc": "紅牌: 卡塞米羅", "sc": "红牌: 卡塞米罗"}},
                    {"minute": 78, "type": "goal", "text": {"en": "Goal: Salah", "tc": "進球: 薩拉赫", "sc": "进球: 萨拉赫"}}
                ]
                # Sort DESC
                events.sort(key=lambda x: x['minute'], reverse=True)
                
        # Real Data Fetch logic remains...
            ...
            if details:
                ...
                for g in details.get('goals', []):
                    ...
                    events.append({
                        "minute": g['minute'], 
                        "type": "goal", 
                        "text": {"en": f"Goal: {scorer}", "tc": f"進球: {scorer}", "sc": f"进球: {scorer}"}
                    })
                for b in details.get('bookings', []):
                    ...
                    events.append({
                        ...
                        "type": "card", 
                        "card": card_en, 
                        "text": {"en": f"Card: {player}", "tc": f"{card_tc}: {player}", "sc": f"{card_sc}: {player}"}
                    })
                events.sort(key=lambda x: x['minute'], reverse=True)
