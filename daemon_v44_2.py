...
def update_status(state, msg, detail=None):
    data = {
        "status": state,
        "message": msg,
...
def push_to_server(content):
    update_status("PUSHING", "正在推送 HTML 到伺服器")
    try:
        ...
        update_status("IDLE", "推送完成，進入休眠", {"last_push": res.status_code})
    except Exception as e:
        update_status("ERROR", f"推送失敗: {str(e)}")
...
def fetch_slow():
    update_status("FETCHING_SLOW", "正在抓取積分榜與新聞")
    standings = {}
...
def fetch_fast():
    update_status("FETCHING_FAST", "正在抓取即時賽事")
    live = []
    upcoming = []
    try:
        ...
        if r.status_code == 200:
            for m in r.json().get('matches', []):
                ...
                if m['status'] in ['IN_PLAY', 'PAUSED']:
                    # Rate Limit Handling for Details
                    time.sleep(0.5) 
                    try:
...
def generate_and_push():
    ...
    # HTML Generation (Same as v44.1 template)
...
<div style="text-align:center; padding:30px; color:#666;">v44.2 Auto-Daemon (Observable)</div>
...
function openNews(n) {{
    document.getElementById('modal-content').innerHTML = `<h3>${{n.title}}</h3><img src="${{n.img}}" style="width:100%;"><p>${{n.body}}</p><button onclick="document.getElementById('myModal').style.display='none'" style="width:100%; margin-top:20px; padding:10px; background:#333; color:#fff; border:none;">關閉</button>`;
    document.getElementById('myModal').style.display = 'block';
}}
...
if __name__ == "__main__":
    try:
        ...
        update_status("FATAL", f"程序崩潰: {str(e)}")
        traceback.print_exc()
