...
# EXPANDED DICTIONARY (Final Polish)
I18N = {
    "teams": {
        ...
        "Ipswich": "伊普斯維奇", "Leicester": "萊斯特城", "Southampton": "南安普頓",
        "Leeds": "里茲聯", "Girona": "赫羅納", "Real Sociedad": "皇家社會", "Betis": "貝蒂斯",
        "Athletic": "畢爾包", "Valencia": "瓦倫西亞", "Villarreal": "比利亚雷亚尔",
        "Sevilla": "塞維利亞", "Osasuna": "奧薩蘇納", "Getafe": "赫塔費", "Celta": "塞爾塔",
        "Mallorca": "馬略卡", "Rayo Vallecano": "巴列卡諾", "Las Palmas": "拉斯帕爾馬斯",
        "Alavés": "阿拉維斯", "Leganés": "雷加利斯", "Valladolid": "瓦拉多利德", "Espanyol": "西班牙人",
        ...
        "Club Brugge": "布魯日", "Shakhtar": "礦工", "Salzburg": "薩爾茨堡"
    },
    "status": {
...
def translate(txt, cat='teams'):
    ...
    for k, v in I18N[cat].items():
        if k in txt: return v # Partial match
    return txt
...
def run():
    ...
    for m in matches:
        ...
        # TIME FIX: User reports -1hr error. Force +1hr correction.
        # UTC+8 (Beijing) + 1hr (Correction) = UTC+9 total shift from raw API timestamp
        utc_dt = datetime.strptime(m['utcDate'], "%Y-%m-%dT%H:%M:%SZ")
        bj_dt = utc_dt + timedelta(hours=8) + timedelta(hours=1) 
        time_str = bj_dt.strftime('%H:%M')
        date_str = bj_dt.strftime('%m/%d')
        
        # DEBUG: If user says it's still wrong, I might need +9 (Japan/Korea time?) or daylight savings issue?
        # But Beijing is strictly UTC+8 no DST.
        # London is UTC+0 in winter.
        # 21:00 London = 05:00 Beijing.
        # My code: utc_dt (21:00) + 8 hours = 05:00. This should be correct.
        # If I saw 04:00, maybe server time was wrong? I use utc_dt object.
