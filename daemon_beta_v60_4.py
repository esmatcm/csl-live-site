...
def run():
    ...
    for m in matches:
        ...
        if status in ['IN_PLAY', 'PAUSED', 'FINISHED']:
            ...
            if details:
                ...
                for p in details.get('homeTeam', {}).get('bench', []):
                    lineups['bench_home'].append(p['name'])
                for p in details.get('awayTeam', {}).get('bench', []):
                    lineups['bench_away'].append(p['name'])
                
                # Coach
                if details.get('homeTeam', {}).get('coach'): coach['home'] = details['homeTeam']['coach']['name']
                if details.get('awayTeam', {}).get('coach'): coach['away'] = details['awayTeam']['coach']['name']

                # Referees
                for r in details.get('referees', []):
                    referees.append(r['name'])

        # 2. Simulation Injector (For Testing UI)
        if status == 'IN_PLAY' and not events:
            events = [
                {"minute": 78, "type": "goal", "text": {"en": "Goal: Salah", "tc": "進球: 薩拉赫", "sc": "进球: 萨拉赫"}},
                {"minute": 56, "type": "card", "card": "red", "text": {"en": "Red Card: Casemiro", "tc": "紅牌: 卡塞米羅", "sc": "红牌: 卡塞米罗"}},
                {"minute": 12, "type": "goal", "text": {"en": "Goal: Haaland", "tc": "進球: 哈蘭德", "sc": "进球: 哈兰德"}}
            ]
            lineups['home'] = ['Ederson', 'Walker', 'Dias', 'Ake', 'Rodri', 'De Bruyne', 'Foden', 'Haaland']
            lineups['away'] = ['Alisson', 'Trent', 'Van Dijk', 'Konate', 'Mac Allister', 'Szoboszlai', 'Salah', 'Nunez']
            coach['home'] = "Pep Guardiola"
            coach['away'] = "Jurgen Klopp"
    ...
    # Force Push Dummy Match for UI Testing (Requested by User)
    dummy_match = {
        "id": 999999,
        "league": {"en": "Premier League (Test)", "tc": "英超 (測試)", "sc": "英超 (测试)"},
        ...
        "lineups": {
            "home": ["Ederson", "Walker", "Dias"],
            "away": ["Alisson", "Van Dijk", "Salah"],
            "bench_home": ["Ortega", "Stone"],
            "bench_away": ["Kelleher", "Gakpo"]
        },
        "referees": ["Michael Oliver"],
        "coach": {"home": "Pep Guardiola", "away": "Jurgen Klopp"}
    }
    data_out.insert(0, dummy_match) # Put at top
