...
def push():
    print("Pushing DEBUG...")
    try:
        with open('csl-live-site/debug_page.html', 'r', encoding='utf-8') as f:
            content = f.read()
            
        now_time, token = get_bt_token()
        url = f"{BT_PANEL_URL}/files?action=SaveFileBody"
        payload = {'request_time': now_time, 'request_token': token, 'path': "/www/wwwroot/hsapi.xyz/debug.html", 'data': content, 'encoding': 'utf-8'}
