...
# sys.stdout.reconfigure(encoding='utf-8') # Removed to prevent crash
...
def run():
    # 1. Fetch Matches (Range: Past 3 Days -> Future 7 Days) to ensure content
    today = datetime.now()
    past_days = (today - timedelta(days=3)).strftime('%Y-%m-%d')
    ...
    # Push HTML
    try:
        html_path = 'csl-live-site/index_quantum_fixed.html'
        if not os.path.exists(html_path):
            # Fallback if specific file missing
            html_path = 'csl-live-site/index_final.html'
            
        with open(html_path, 'r', encoding='utf-8') as f:
            html = f.read()
        push(html, PATH_INDEX)
    except Exception as e:
        print(f"HTML Push Error: {e}")

if __name__ == "__main__":
    print("Daemon v71 Starting...")
    run()
    while True:
        time.sleep(30)
        run()
