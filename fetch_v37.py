...
def fetch_data():
    ...
    print("Fetching News (Using Cached Files)...")
    news = []
    try:
        ...
        # 這裡只負責生成 JSON 連結，圖片由 deploy_images.py 上傳
        for i, (href, title) in enumerate(matches[:8]):
            # 假設圖片已上傳成功，直接使用相對路徑
            img_path = f"img_cache/news_{i}.jpg"
            news.append({"id": i, "title": title.strip(), "img": img_path, "url": href})
    except: pass
...
<head>
    ...
    <style>
        ...
        .news-img {{ width: 80px; height: 60px; object-fit: cover; border-radius: 8px; background: #333; }}
...
<body>
    ...
    <div id="live-container">
        {''.join([f'''
        <div class="card live" onclick="alert('靜態模式：暫不提供實時詳情')">
    ...
    <div class="card">
        ...
        <div class="news-item" onclick="window.open('{n['url']}', '_blank')">
            <img src="{n['img']}" class="news-img" onerror="this.src='https://via.placeholder.com/80?text=Error'">
    ...
    <div class="footer">
        Build: v37.0 (Image Upload Fixed)<br>
        Detail API Disabled
    </div>
    <script>console.info("Static build: match detail disabled");</script>
</body>
</html>"""
