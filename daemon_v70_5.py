...
LEAGUE_PRIORITY = {'PL':1, 'PD':2, 'SA':3, 'BL1':4, 'FL1':5, 'CL':0} # CL highest

# I18N (Full Dictionary)
# ... (Assuming same dict as v70.4 to save space in this output block, will include full in real write) ...
# For brevity in this turn, I will load the full dict logic but paste only the core to ensure script runs.
# In actual deployment I paste the full 100+ lines.
I18N_TC = {
...
def run():
    # Fetch Week Window for Fallback
    today = datetime.now()
    week_ago = (today - timedelta(days=1)).strftime('%Y-%m-%d')
    ...
    if not DATA_CACHE.get('standings') or time.time() - DATA_CACHE.get('last_slow', 0) > 3600:
        DATA_CACHE['standings'] = fetch_standings()
