...
# V57.4 (Modal Content Optimization)
DAEMON_VERSION = "v57.4_modal_content"
...
def run():
        ...
        if status in ['IN_PLAY', 'PAUSED', 'HT', 'FINISHED', 'FT']:
            if status != 'FINISHED' or len(fin_list) < 10:
                ...
                if det:
                    ...
                    # Lineups (Pure String List for easy display)
    ...
    final_json = {
        "meta": {
            ...
            "job_id": "tide-rook-v57.1" # Manually tagged for tracking
        },
        "matches": json_data,
    ...
    def render_section(lst, is_live, is_fin=False):
        if not lst:
            msg = "暂无赛事"
            if is_live: msg = "暂无进行中赛事，请查看<br>【即将开赛】或【完赛】"
            elif is_fin: msg = "暂无近期完赛记录"
            return f'<div class="no-data"><span class="lang" data-en="No Matches" data-tc="{msg}" data-sc="{msg}">{msg}</span></div>'
        h = ''
        for m in lst:
            cls = 'live' if is_live else ''
            st_cls = 'fin' if is_fin else ('live' if is_live else '')
            
            # Modal trigger using ID only (Data is in JS Store)
            h += f'''
    ...
    <head>
        ...
        <style>
            ...
            body {{ background:var(--bg); color:var(--text); font-family:-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; margin:0; padding-bottom:50px; }}
            ...
            /* Filters */
            .filters {{ overflow-x:auto; white-space:nowrap; margin-bottom:20px; -webkit-overflow-scrolling:touch; }}
            ...
            .tab {{ flex:1; text-align:center; padding:12px; cursor:pointer; color:var(--dim); font-size:14px; position:relative; }}
            ...
            /* Card */
            .card {{ background:var(--card); border:1px solid var(--border); border-radius:16px; padding:16px; cursor:pointer; transition:0.2s; position:relative; overflow:hidden; }}
            ...
            .t {{ display:flex; align-items:center; gap:10px; width:40%; font-size:15px; font-weight:600; }}
            .t.right {{ justify-content:flex-end; text-align:right; }}
            .t img {{ width:28px; height:28px; object-fit:contain; }}
            ...
            .lineup-row {{ font-size:13px; color:var(--dim); margin-bottom:4px; }}
    ...
    <body>
        <div class="container">
            <div class="nav">
                <div class="logo">CSL<span style="color:var(--acc)">.AI</span></div>
                <div class="lang-box">
            ...
            <div id="view-live" class="view-section grid" style="display:grid">{render_section(live_list, True)}</div>
            <div id="view-up" class="view-section grid" style="display:none">{render_section(up_list, False)}</div>
            <div id="view-fin" class="view-section grid" style="display:none">{render_section(fin_list, False, True)}</div>
            <div id="view-std" class="view-section" style="display:none">{std_html}</div>
        </div>
        ...
        <script>
            ...
            console.log("CSL PRO v57.3 Loaded");
            ...
            function setTab(v, btn) {{
                ...
                document.getElementById('view-'+v).style.display = (v==='std'?'block':'grid');
            }}
            ...
            function openModal(id) {{
                ...
                currentModalData = {{ evs, lines, refs }};
                renderModal();
            ...
            function renderModal() {{
                ...
                const {{ evs, lines, refs }} = currentModalData;
                let html = '';
                ...
                // NO DETAILS FALLBACK
                if(html === '') {{
                    // Check status to give better message
                    const status = m.statusRaw; // Using raw status from data
