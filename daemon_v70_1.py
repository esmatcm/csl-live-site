...
# PATHS (Reuse known existing files to avoid permission errors)
PATH_INDEX = "/www/wwwroot/hsapi.xyz/index.html"
PATH_FULL = "/www/wwwroot/hsapi.xyz/full.html"
PATH_DATA = "/www/wwwroot/hsapi.xyz/data.json" # Standard Name

# Filter Config (Frontend will use this)
ALLOWED = ['PL', 'PD', 'SA', 'BL1', 'FL1', 'CL']

# I18N (v55 Full Dictionary)
I18N_TC = {
    ...
    "St. Pauli": "聖保利", "Holstein Kiel": "基爾", "Stuttgart": "斯圖加特"
}
I18N_SC = {
...
def run():
    ...
    for m in matches:
        ...
        bj_dt = utc_dt + timedelta(hours=9) # Japan
    ...
    # Push Single Data Source
    push(json.dumps(data_all, ensure_ascii=False), PATH_DATA)
    ...
    try:
        ...
        # PROD HTML: Inject Strict Filter Logic
        # We replace the default "return true" in the filter with a check
        filter_logic = "if (!['PL','PD','SA','BL1','FL1','CL'].includes(m.code)) return false;"
        html_prod = html_base.replace('return true;', filter_logic + ' return true;')
        # Also remove title suffix
        html_prod = html_prod.replace('CSL PRO QUANTUM', 'CSL PRO')
        
        push(html_prod, PATH_INDEX)
        
        # FULL HTML: No Filter
        push(html_base, PATH_FULL)
