...
def push_log():
    print("Pushing Update Log...")
    try:
        with open('csl-live-site/UPDATE_LOG.md', 'r', encoding='utf-8') as f:
            content = f.read()
            
        now_time, token = get_bt_token()
        url = f"{BT_PANEL_URL}/files?action=SaveFileBody"
        payload = {'request_time': now_time, 'request_token': token, 'path': "/www/wwwroot/hsapi.xyz/UPDATE_LOG.md", 'data': content, 'encoding': 'utf-8'}
        
        r = requests.post(url, data=payload, timeout=10)
        print(f"Log Push Status: {r.status_code}")
