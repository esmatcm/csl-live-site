    ...
    for code in ['PL', 'PD', 'SA', 'BL1', 'FL1']:
        try:
            ...
            if r.status_code == 200:
                table = []
                for row in r.json()['standings'][0]['table'][:10]:
...
def run():
    ...
    # 2. Fetch Standings (If needed, simple cache logic)
    ...
    # 3. Construct Final JSON Object (Matches + Standings)
    final_json = {
        "matches": data_all,
    ...
    try:
        with open('csl-live-site/index_quantum_fixed.html', 'r', encoding='utf-8') as f:
            html = f.read()
        push(html, PATH_INDEX) # Prod
        push(html, PATH_FULL)  # Full (same UI, JS handles filtering)
    except: pass
