...
PATH_HTML_TYPO = "/www/wwwroot/hsapi.xyz/bata.html" # Handle typo
...
def run():
    ...
    for m in matches:
        ...
        # Lineups & Referees (Tier 3)
        lineups = {"home": [], "away": []}
        referees = []
        
        if status in ['IN_PLAY', 'PAUSED', 'FINISHED']: # Fetch details for finished too
            details = fetch_match_details(mid)
            if details:
                # Events (Goals/Cards)
                for g in details.get('goals', []):
                    scorer = g['scorer']['name'] if g.get('scorer') else "Unknown"
                    events.append({"minute": g['minute'], "type": "goal", "text": f"Goal: {scorer}"})
                for b in details.get('bookings', []):
                    player = b['player']['name']
                    events.append({"minute": b['minute'], "type": "card", "card": b['card'].lower().replace('_card',''), "text": f"Card: {player}"})
    ...
    try:
        with open('csl-live-site/index_quantum_fixed.html', 'r', encoding='utf-8') as f:
            html = f.read()
        push(html, PATH_HTML)
        push(html, PATH_HTML_TYPO) # Push to bata.html too
    except: pass
