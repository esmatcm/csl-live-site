...
PATH_BETA_HTML = "/www/wwwroot/hsapi.xyz/bata.html"
PATH_BETA_DATA = "/www/wwwroot/hsapi.xyz/data_v60.json"
...
def run():
    ...
    dummy = {
        ...
        "streams": [
            {"name": {"en": "Zhibo8", "tc": "直播吧", "sc": "直播吧"}, "url": "https://www.zhibo8.com"},
            {"name": {"en": "88Kanqiu", "tc": "88看球", "sc": "88看球"}, "url": "http://www.88kanqiu.one"}
        ]
    }
    data_out.insert(0, dummy)

    push(json.dumps(data_out, ensure_ascii=False), PATH_BETA_DATA)
    
    # Push HTML Template (Read local Quantum fixed)
    try:
        with open('csl-live-site/index_quantum_fixed.html', 'r', encoding='utf-8') as f:
            html = f.read()
        push(html, PATH_BETA_HTML)
    except: pass
